import numpy as np
from pathlib import Path
from typing import Tuple, Optional
from tqdm import tqdm

from data.preprocessor import ImagePreprocessor
from features.color_features import ColorFeatureExtractor
from features.texture_features import TextureFeatureExtractor
from features.shape_features import ShapeFeatureExtractor


class FeatureExtractor:
    """Orchestrates extraction of selected feature types."""

    VALID_GROUPS = {"hsv", "glcm", "shape"}

    def __init__(self, config):
        self.config = config

        self.selected_groups = self._get_selected_groups()

        self.preprocessor = ImagePreprocessor(config)

        self.color_extractor = ColorFeatureExtractor(
            bins=tuple(config.get("features.hsv_bins", [8, 8, 4]))
        )

        self.texture_extractor = TextureFeatureExtractor(
            glcm_size=tuple(config.get("features.glcm_size", [128, 128])),
            levels=config.get("features.glcm_levels", 8),
            distances=config.get("features.glcm_distances", [1]),
            angles=config.get(
                "features.glcm_angles",
                [0, np.pi / 4, np.pi / 2, 3 * np.pi / 4],
            ),
        )
        # Otomatis menyala jika segmentasi aktif, otomatis mati jika segmentasi mati
        is_segmented = config.get("segmentation.enabled", False)
        
        self.shape_extractor = ShapeFeatureExtractor(
            extract_area_features=is_segmented
        )

    def _get_selected_groups(self) -> list:
        """
        Read selected feature groups from config.

        Example:
            features:
              selected_groups: ["hsv"]
              selected_groups: ["glcm"]
              selected_groups: ["hsv", "glcm"]
              selected_groups: ["hsv", "glcm", "shape"]
        """
        selected = self.config.get(
            "features.selected_groups",
            ["hsv", "glcm", "shape"],
        )

        if isinstance(selected, str):
            selected = [selected]

        alias = {
            "color": "hsv",
            "warna": "hsv",
            "texture": "glcm",
            "tekstur": "glcm",
            "bentuk": "shape",
            "area": "shape",
        }

        normalized = []
        for group in selected:
            group = str(group).lower().strip()
            group = alias.get(group, group)

            if group not in self.VALID_GROUPS:
                raise ValueError(
                    f"Feature group tidak dikenal: {group}. "
                    f"Gunakan salah satu dari: {sorted(self.VALID_GROUPS)}"
                )

            if group not in normalized:
                normalized.append(group)

        if not normalized:
            raise ValueError("features.selected_groups tidak boleh kosong.")

        return normalized

    def extract_features_from_pair(
        self,
        before_path: Path,
        after_path: Path,
    ) -> Optional[np.ndarray]:

        img_before, img_after, mask_before, mask_after = (
            self.preprocessor.process_image_pair(before_path, after_path)
        )

        if img_before is None or img_after is None:
            return None

        if self.config.get("segmentation.enabled", False):
            img_before_masked = self.preprocessor.apply_mask_to_image(
                img_before, mask_before
            )
            img_after_masked = self.preprocessor.apply_mask_to_image(
                img_after, mask_after
            )
        else:
            img_before_masked = img_before
            img_after_masked = img_after
            mask_before = None
            mask_after = None

        feature_parts = []

        if "hsv" in self.selected_groups:
            color_features = self.color_extractor.extract_pair_features(
                img_before_masked,
                img_after_masked,
            )
            feature_parts.append(color_features)

        if "glcm" in self.selected_groups:
            texture_features = self.texture_extractor.extract_pair_features(
                img_before_masked,
                img_after_masked,
            )
            feature_parts.append(texture_features)

        if "shape" in self.selected_groups:
            shape_features = self.shape_extractor.extract_pair_features(
                img_before_masked,
                img_after_masked,
                mask_before,
                mask_after,
            )
            feature_parts.append(shape_features)

        features = np.concatenate(feature_parts, axis=0)
        return features.astype(np.float32)

    def extract_features_from_dataset(
        self,
        dataframe,
        show_progress: bool = True,
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, list]:

        features_list = []
        valid_indices = []
        skipped = []

        iterator = (
            tqdm(dataframe.iterrows(), total=len(dataframe))
            if show_progress
            else dataframe.iterrows()
        )

        for idx, row in iterator:
            try:
                features = self.extract_features_from_pair(
                    row["before_path"],
                    row["after_path"],
                )

                if features is not None and np.all(np.isfinite(features)):
                    features_list.append(features)
                    valid_indices.append(idx)
                else:
                    skipped.append((idx, "Invalid features (NaN or Inf)"))

            except Exception as e:
                skipped.append((idx, str(e)))

        if len(features_list) == 0:
            raise RuntimeError(
                "Tidak ada fitur valid yang berhasil diekstraksi. "
                "Cek path gambar, konfigurasi preprocessing, dan selected_groups."
            )

        X = np.vstack(features_list)
        y = dataframe.loc[valid_indices, "y_percent_leftover"].to_numpy().astype(np.float32)
        groups = dataframe.loc[valid_indices, "group"].to_numpy()

        return X, y, groups, skipped

    def get_feature_names(self) -> list:
        names = []

        if "hsv" in self.selected_groups:
            names.extend(self.color_extractor.get_feature_names())

        if "glcm" in self.selected_groups:
            names.extend(self.texture_extractor.get_feature_names())

        if "shape" in self.selected_groups:
            names.extend(self.shape_extractor.get_feature_names())

        return names

    def get_feature_dimensions(self) -> dict:
        color_dim = self.color_extractor.feature_dim * 3
        texture_dim = len(self.texture_extractor.properties) * 3
        shape_dim = len(self.shape_extractor.get_feature_names())

        dims = {
            "hsv": color_dim if "hsv" in self.selected_groups else 0,
            "glcm": texture_dim if "glcm" in self.selected_groups else 0,
            "shape": shape_dim if "shape" in self.selected_groups else 0,
        }

        dims["total"] = dims["hsv"] + dims["glcm"] + dims["shape"]
        return dims